...
private:
    void setupUI() {
        ...
        // 设置初始目录
        QModelIndex homeIndex = dirModel->index(QDir::homePath());
        treeView->expand(homeIndex);
        treeView->setCurrentIndex(homeIndex);
        tableView->setRootIndex(fileModel->index(QDir::homePath()));
        updateAddressBar(QDir::homePath());

        // 初始化历史记录
        history.append(QDir::homePath());
        historyIndex = 0;
